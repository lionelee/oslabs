...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	void *cur = ROUNDDOWN(va, PGSIZE), *end = ROUNDUP(va+len, PGSIZE);
	while(cur < end){
		struct Page* pg = page_alloc(0);
    if (pg == NULL) panic("failed to alloc region\n");
		if(page_insert(e->env_pgdir, pg, cur, PTE_W | PTE_U) < 0)
