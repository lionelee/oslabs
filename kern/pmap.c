...
mem_init(void)
{
	...
	//uint32_t ksize = 0xFFFFFFFF;
	//ksize = ksize - KERNBASE + 1;
	//boot_map_region_large(kern_pgdir, KERNBASE, ksize, 0, PTE_W);
	...
	// Enable page size extension(PSE) before loading cr3
  uint32_t cr4 = rcr4();
  cr4 |= CR4_PSE;
  lcr4(cr4);
...
mem_init_mp(void)
{
	...
	for(int i = 0; i < NCPU; ++i){
		uintptr_t kstackbtm_i = KSTACKTOP - i * (KSTKSIZE + KSTKGAP) - KSTKSIZE;
		boot_map_region(kern_pgdir, kstackbtm_i, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W);
	}
...
page_init(void)
{
	...
	//int npages_ext_free =  ((uint32_t)boot_alloc(0)- KERNBASE) / PGSIZE;	//allocated pages
	//int idx_ext_free = npages_basemem + (EXTPHYSMEM - IOPHYSMEM) / PGSIZE + npages_ext_free;

	size_t idx_ext_free = PADDR(boot_alloc(0)) / PGSIZE;
	pages[0].pp_ref = 1; pages[0].pp_link = NULL;
	for (size_t i = 1; i < npages; i++) {
		if((i < npages_basemem && i != MPENTRY_PADDR/PGSIZE) || i >= idx_ext_free){
			pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
	...
	pg->pp_link = NULL;
	if (pg->pp_ref != 0)
  	panic("cant't allocate a page in use\n");
...
page_free(struct Page *pp)
{
	// Fill this function in
	if(pp->pp_ref != 0)
		panic("can't free a page in use");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	pde_t* pde = pgdir + pdx;
	if(*pde & (PTE_PS&PTE_P)) return pde;
	if(!(*pde & PTE_P)){	//relevant page-table page not present
...
boot_map_region_large(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	while(i < size){
		...
		*pte = PTE_ADDR(pa) | perm | PTE_PS | PTE_P;
		va += PTSIZE;
		pa += PTSIZE;
		i += PTSIZE;
	}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	++pp->pp_ref;
	if(*pte & PTE_P){
		page_remove(pgdir, va);	//remove mapped page at 'va'
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	struct Page* pg = pa2page(PTE_ADDR(*pte));
