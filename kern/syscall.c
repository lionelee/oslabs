	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if(srcva >= (void*)UTOP || PGOFF(srcva) != 0 || dstva >= (void*)UTOP
			|| PGOFF(dstva)!=0 || (perm & ~PTE_SYSCALL) != 0)
		return -E_INVAL;
	perm |= (PTE_U | PTE_P);
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if(srcva < (void*)UTOP){
		if(PGOFF(srcva) != 0 || (perm & ~PTE_SYSCALL) != 0)
			return -E_INVAL;
		perm |= (PTE_U | PTE_P);
		pte_t* pte = NULL;
		struct Page *pg = page_lookup(curenv->env_pgdir, srcva, &pte);
		if(pg == NULL || ((perm & PTE_W) && (*pte & PTE_W) == 0))
			return -E_INVAL;
...
int32_t
syscall_wrap(struct Trapframe *tf) {
  int32_t res;
  lock_kernel();
  curenv->env_tf = *tf;
  res = syscall(tf->tf_regs.reg_eax, tf->tf_regs.reg_edx, tf->tf_regs.reg_ecx,
                tf->tf_regs.reg_ebx, tf->tf_regs.reg_edi, 0);
  unlock_kernel();
  return res;
}
