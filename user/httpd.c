...
send_data(struct http_request *req, int fd)
{
	...
	struct Stat stat;
	r = fstat(fd, &stat);
	if(r < 0) die("send_data: fstat failed\n");
	if (stat.st_size > 1518)
	  die("send_data: size larger than 1518\n");

	r = readn(fd, buf, stat.st_size);
	if(r != stat.st_size) die("send_data: read data failed\n");
	r = write(req->sock, buf, stat.st_size);
	if(r != stat.st_size) die("send_data: write data failed\n");
	return 0;
}
